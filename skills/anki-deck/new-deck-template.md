---
name: anki-{{SLUG}}
description: Add Anki flashcards to the "{{DECK_NAME}}" deck ({{DECK_DESCRIPTION}}). Use when the user wants to create or add cards for {{DECK_NAME}}.
---
...
│  ├─ Tool not found (no anki MCP tools exist) → [A0] → STOP
...
│  │  │  └─ NO → inputType = TEXT
│  │  │
│  │  Step 2: Parse text portion (from the user's request or image analysis result)
...
2. **Choices block** — conditional: `{{#Choices}}...{{/Choices}}`
   ...
   - Each choice on its own line, distinctly numbered with ①②③④⑤.
...
- **Answer / CorrectChoice:** medium weight — legibly readable but not as heavy as the question.
...
The user's request contains the text input. The user may also attach images.
...
- All choices must be plausible. No blatantly wrong "joke" answers.
...
The user's request — topic(s) optionally followed by a mode keyword and/or choice specs. The user may also attach images as input sources. Parse all input per A4 rules, then create cards.
